...
public class TokenBucketPolicy implements Policy {
    ...
    public LoadShedDecision decide(RequestContext request, SystemSnapshot snapshot) {
        ...
        if (bucket != null && bucket.tryConsume(request.estimatedCost())) {
            return new LoadShedDecision(DecisionType.ACCEPT, DecisionReason.HEALTHY, 1.0, "Token available");
        }
        return new LoadShedDecision(DecisionType.REJECT, DecisionReason.RATE_LIMITED, 0.0, "No tokens");
    }
    private static class Bucket {
        private final double capacity, rate;
        private double tokens;
        private long lastRefillTimestamp;
        Bucket(double capacity, double rate) { this.capacity = capacity; this.rate = rate; this.tokens = capacity; this.lastRefillTimestamp = System.nanoTime(); }
        ...
        private void refill() {
            ...
            if (elapsedTime > 0) { tokens = Math.min(capacity, tokens + elapsedTime * rate); lastRefillTimestamp = now; }
        }
    }
}
